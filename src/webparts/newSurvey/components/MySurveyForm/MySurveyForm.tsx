...
import { Box, Button, Checkbox, FormControl, FormControlLabel, FormGroup, Radio, RadioGroup, Rating, Typography } from "@mui/material";
...
import ViewResponseData from "./ViewResponseData/ViewResponseData";
...
const MySurveyForm = (props: IMySurveyForm) => {
  ...
  const [answer4, setAnswer4] = useState<number>(0);
  ...
  const answer2Error = useMemo(() => {
    if (answer2.length < 2 && answer2.length > 0 && _currentQuestion === 2) {
      return "Please select at least 2 programming languages";
    } else {
      return "";
    }
  }, [answer2])
  ...
  const handleSubmit = async () => {
    const data = {
      ...
      Skills: answer2,
      DateOfBirth: answer3,
      RatingSurvey: answer4,
    }
    await _sp.web.lists.getByTitle('New Survey Form').items.add(data);
    setIsAnsweredSurvey(true);
  }
        ...
        <Box>
          {
            isAnsweredSurvey ?
              ...
              >
                <ViewResponseData context={props.context} listName={ListName} />
              </Box> :
              <Box
                sx={{
                  display: 'flex',
                        ...
                        <Button variant="contained" color="success"
                          onClick={() => {
                            if (_currentQuestion === 4) {
                              ...
                              handleNextQuestion(_currentQuestion);
                            }
                          }}
                          disabled={_currentQuestion === 2 && answer2.length < 2}
                        >
                          {
